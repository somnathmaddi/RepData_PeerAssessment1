...
hist(stepsPerDay$steps, breaks = nrow(stepsPerDay), main = "Total Number of Steps Each Day", xlab = "Steps each Day", col = "Orange")
```
...
plot(x=perIntervalStepsAverage$interval,y=perIntervalStepsAverage$steps,type='l',main = "Average Daily Time Interval Activity Pattern", xlab = "5-minute Intervals", ylab = "Average Steps")
...
maxInterval<-perIntervalStepsAverage[which.max(perIntervalStepsAverage$steps),"interval"]

```


#### Top 5-minute interval, on average across all the days in the dataset, Which  contains the maximum number of steps is `r maxInterval`
...
## create a new dataset that is equal to the original dataset without missing data
...
hist(imputedStepsPerDay$steps,
     breaks = nrow(imputedStepsPerDay),
...
#### The mean of total number of steps taken per day is `r imputedMeanSteps`
#### The mean of total number of steps taken per day is `r imputedmedianSteps`
...
imputedActivity$day_type <- as.factor(sapply(imputedActivity$date, function(x) ifelse(weekdays(x) %in% weekend, "weekend", "weekday")))
